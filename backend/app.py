...
def create_charts():
    ...
    plt.tight_layout(); plt.savefig("images/type.png"); plt.close()

    plt.figure(figsize=(6, 4))
    year_counts = df['year_added'].value_counts().sort_index()
    plt.plot(year_counts.index, year_counts.values, marker='o', color='#00FFFF', linewidth=2)
    plt.title("Content Added Per Year", fontsize=14, fontweight='bold')
    plt.xlabel("Year"); plt.ylabel("Number of Titles")
    plt.grid(True, linestyle='--', alpha=0.3)
    plt.tight_layout(); plt.savefig("images/year.png"); plt.close()
    ...
    plt.xlabel("Number of Titles"); plt.gca().invert_yaxis()
    plt.grid(axis='x', linestyle='--', alpha=0.3)
    for bar in bars:
        width = bar.get_width()
        plt.text(width+5, bar.get_y()+bar.get_height()/2, int(width), va='center')
    plt.tight_layout(); plt.savefig("images/country.png"); plt.close()
    ...
    for bar in bars:
        yval = bar.get_height()
        plt.text(bar.get_x()+bar.get_width()/2, yval, int(yval), ha='center', va='bottom')
    plt.tight_layout(); plt.savefig("images/genre.png"); plt.close()
    ...
    for bar in bars:
        yval = bar.get_height()
        plt.text(bar.get_x()+bar.get_width()/2, yval, int(yval), ha='center', va='bottom')
    plt.tight_layout(); plt.savefig("images/rating.png"); plt.close()
...
VALID_RATINGS = ["TV-MA", "TV-14", "TV-PG", "PG-13", "R", "PG",
                 "TV-Y", "TV-Y7", "TV-G", "G", "NC-17", "NR", "UR", "TV-Y7-FV"]
...
data["genre_enc"]   = genre_encoder.fit_transform(data["listed_in"].str.split(",").str[0].str.strip())
data["country_enc"] = country_encoder.fit_transform(data["country"].str.split(",").str[0].str.strip())

flag_cols    = [c for c in data.columns if c.startswith("has_") or c in ["dur_short","dur_long"]]
numeric_cols = [
    "genre_enc", "country_enc", "dur_num", "release_year",
    "is_movie", "is_tv",
] + flag_cols
...
for rating_val in data_filtered["rating"].unique():
    ...
    if len(subset) > sample_size:
        ...
        if len(subset) < sample_size // 2 and len(subset) >= 6:
            oversampled = subset.sample(n=sample_size // 2, replace=True, random_state=42)
